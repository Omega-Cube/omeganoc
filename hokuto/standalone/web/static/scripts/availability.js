...
require(['jquery','onoc.createurl', 'console'],function(jQuery,createUrl, console){
    ...
    function buildTimelinePart(title,percent,type,cla){
        if(type === 'host')
            return jQuery('<span title="'+title+'" class="timeline-'+cla+'" style="width:'+percent+'%;"></span>');
        else
            return jQuery('<span title="'+title+'" class="servicetimeline-'+cla+'" style="width:'+percent+'%;"></span>');
    }
    ...
    function requestHostDetails(e){
        var host = jQuery(e.target).data('host');
        ...
        jQuery.ajax({
            ...
            for(var s in response.results){
                ...
                var timelineContainer = jQuery('<p class="timeline"></span>');
                ...
                for(var i = 1, len = response.results[s].timeline.length; i < len; i++){
                    ...
                    percent = time / response.fulltime * 100;
                    title = getPartTitle(response.results[s].timeline[i - 1][0], cur * 1000, response.results[s].timeline[i][1] * 1000, type);
                ...
                percent = (response.end - cur) / response.fulltime * 100;
                title = getPartTitle(response.results[s].timeline[i - 1][0], cur * 1000, response.end * 1000, type);
                timelineContainer.append(buildTimelinePart(title,percent,type,response.results[s].timeline[i - 1][0]));

                container.append(timelineContainer);
                ul.append(container);
            }
            jQuery('#onoc-availability .results').empty().append(ul);
            jQuery('#onoc-availability .results button').on('click', requestHostDetails);
        });
    }
        ...
        jQuery.ajax({
            ...
            for(var h in response.results){
                ...
                for(var i = 1, len = response.results[h].timeline.length; i < len; i++){
                    ...
                    title = getPartTitle(response.results[h].timeline[i - 1][0], cur * 1000, response.results[h].timeline[i][1] * 1000, 'host');
                ...
                percent = (response.end - cur) / response.fulltime * 100;
                title = getPartTitle(response.results[h].timeline[i - 1][0], cur * 1000, response.end * 1000, 'host');
                timelineContainer.append(buildTimelinePart(title, percent, 'host', response.results[h].timeline[i - 1][0]));
            ...
            for(var h in response.results){
                for(var s in response.results[h]){
                    ...
                    for(var i = 1, len = data.timeline.length; i < len; i++){
                        ...
                        title = getPartTitle(data.timeline[i-1][0], cur * 1000, data.timeline[i][1] * 1000, 'service');
                        timelineContainer.append(buildTimelinePart(title,percent,'service',data.timeline[i-1][0]));
                        cur = data.timeline[i][1];
                    }
                    percent = (response.end - cur) / response.fulltime * 100;
                    title = getPartTitle(data.timeline[i-1][0], cur * 1000, response.end * 1000, 'service');
                    timelineContainer.append(buildTimelinePart(title,percent,'service',data.timeline[i-1][0]));
    ...
    jQuery(document).ready(function(){
        ...
        jQuery('#form-hostgroup').on('change', requestHostUpdate);
        jQuery('#form-services').on('change', requestServiceUpdate);
        jQuery('#form-range').on('change', function(){
            if(!_CURRENT) return false;
            if(_CURRENT === 'host') requestHostUpdate();
            if(_CURRENT === 'service') requestServiceUpdate();
        });
        jQuery('#firststate_host').on('change', function(){
            if(!_CURRENT || _CURRENT === 'service') return false;
            requestHostUpdate();
        });
        jQuery('#firststate_service').on('change', function(){
