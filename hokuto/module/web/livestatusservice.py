...
def get_service_formated_logs(host,service,columns):
    ...
    query = query.filter("current_service_contacts >= %s"%shinken_contact)
    query = query.filter("And: 3")
    query = query.filter("service_description = ")
    query = query.filter("current_host_state_type = 1")
    query = query.filter("current_host_contacts >= %s"%shinken_contact)
    query = query.filter("And: 3")
